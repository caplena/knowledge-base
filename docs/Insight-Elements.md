...
| ------------------------------- | ---------------------- |
| **Impact per row**                  | Measuring the impact on the performance metric on an individual basis. <br> Based on multiple regression analysis.     |
...
For the net impact calculation we **combine the driver strength with the frequency of a topic mentioned** allowing us to determine to what extend the NPS (in our example) is influenced overall.

>For the NPS, this is <ins>not</ins> a simple multiplication, but a non-linear step function. If someone’s NPS value changed from 2 to 6, it would not change anything — the person remains a detractor. Therefore, we apply advanced probabilistic modeling to estimate the true net impact and avoid the “randomness” introduced by the step function.
...
The combination of driver strength and number of mentions provides a clearer picture of a topic’s overall contribution to the KPI (such as NPS) across the full dataset.
